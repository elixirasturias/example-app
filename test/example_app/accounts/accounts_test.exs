defmodule ExampleApp.AccountsTest do
  ...
  describe "administrators" do
    ...
    test "list_administrators/0 returns all administrators" do
      _administrator = administrator_fixture()
      assert Enum.count(Accounts.list_administrators()) == 1
    end

    test "get_administrator!/1 returns the administrator with given id" do
      administrator = administrator_fixture()

      assert Accounts.get_administrator!(administrator.id).id ==
               administrator.id
    end
    ...
    test "update_administrator/2 with invalid data returns error changeset" do
      ...
      assert administrator.id ==
               Accounts.get_administrator!(administrator.id).id
    end
